...
def main():
    ...
    parser.add_argument('--base-url', type=str, default='https://dumps.wikimedia.org/{lang}wiki/{date}/')
    ...
    files = ('{name}wiki-{date}-pages-articles-multistream.xml.bz2',
             ...
             '{name}wiki-{date}-md5sums.txt')
    for name in sys.stdin:
        name = name.strip()
        print(f'Downloading {name}...')
        for filename in files:
            filename = filename.format(name=name, date=args.date)
            url = args.base_url.format(lang=name, date=args.date) + filename
