...
def extract_desc(out_fn, langs, eids):
    with open(out_fn, "w") as f:
        for lang in tqdm.tqdm(langs):
            try:
                loader = get_loader(lang)
            except Exception as e:
                print(f"Skip the language {lang} -- Exception: {e}")
                continue
            ...
            for eid in tqdm.tqdm(eids):
                try:
                    ...
                    text = postClean(text)
                    if isDirty(text):
                        print(f"Skip the dirty text: {text}")
                        continue
                    f.write(f"{lang}-{eid}-{pid}\t{text}\n")
                except Exception as e:
                    print(f"Pass the entity {eid} -- Exception: {e}")
                    pass
